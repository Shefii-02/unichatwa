...
export default () => ({
  ...
  api: {
    rateLimit: {
      // Short burst protection: 50 requests per second. The dashboard legitimately bursts well
      // past 10/s — e.g. the profile card resolving a contact's shared groups fans out one
      // /groups/:groupId lookup per group.
      shortTtl: parseInt(process.env.RATE_LIMIT_SHORT_TTL || '1000', 10),
      shortLimit: parseInt(process.env.RATE_LIMIT_SHORT_LIMIT || '50', 10),
