...
export class GroupService {
  ...
  /**
   * The groups this session shares with one contact.
   *
   * Computed here rather than in the browser: the dashboard's "shared groups" panel otherwise
   * fetches every group's member list one HTTP request at a time and trips the API rate limiter
   * (429). Engine reads are not HTTP-rate-limited; this runs them with bounded concurrency and
   * returns a single response.
   *
   * `contactId` may be a `<phone>@c.us`, an `<lid>@lid`, or bare digits. Matching is on the last
   * 10 digits of the phone number, so country-code variants and an @lid whose MSISDN the engine
   * can resolve both line up with the participant list.
   */
  async getGroupsForParticipant(
    ...
    const addKey = (raw: string | null | undefined): void => {
      const digits = (raw ?? '').replace(/[^0-9]/g, '');
      if (digits.length >= 7) keys.add(digits.slice(-10));
    };
    addKey(contactId.replace(/@[a-z.]+$/i, ''));
    try {
      addKey(await engine.resolveContactPhone(contactId));
    } catch {
      // best-effort — the id-derived key still stands
    }
    ...
    for (let i = 0; i < capped.length; i += CONCURRENCY) {
      ...
      const infos = await Promise.all(slice.map(g => engine.getGroupInfo(g.id).catch(() => null)));
      infos.forEach((info, j) => {
        const isMember = info?.participants?.some(p => {
          const pd = (p.number ?? '').replace(/[^0-9]/g, '');
          return pd.length >= 7 && keys.has(pd.slice(-10));
        });
        if (isMember) matched.push({ id: slice[j].id, name: slice[j].name });
      });
    }
    return matched;
  }
