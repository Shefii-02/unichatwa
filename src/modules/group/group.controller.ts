...
export class GroupController {
  ...
  // MUST stay above @Get(':groupId') for the same reason as join-info — a literal segment declared
  // after the parameter route is shadowed by it.
  @Get('for-contact')
  @ApiOperation({
  ...
  @ApiParam({ name: 'sessionId', description: 'Session ID' })
  @ApiQuery({ name: 'contactId', description: 'Contact id — <phone>@c.us, <lid>@lid, or bare digits' })
  @ApiQuery({ name: 'limit', required: false, description: 'Max groups to scan (default 100)' })
  @ApiResponse({ status: 200, description: 'Groups the contact is a participant of', type: [GroupSummaryDto] })
  ...
  async getSharedGroups(
    ...
    return this.groupService.getGroupsForParticipant(sessionId, contactId ?? '', {
