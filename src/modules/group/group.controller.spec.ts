...
describe('GroupController route ordering', () => {
  it('declares the literal routes before the :groupId parameter route', () => {
    const proto = GroupController.prototype as object;
    const order = Object.getOwnPropertyNames(proto).filter(
      name => name === 'joinInfo' || name === 'getSharedGroups' || name === 'findOne',
    );

    expect(order).toEqual(['joinInfo', 'getSharedGroups', 'findOne']);
  });
  ...
  it('keeps for-contact a literal path, not a parameter', () => {
    const handler = Object.getOwnPropertyDescriptor(GroupController.prototype, 'getSharedGroups')?.value as object;
    const path: unknown = Reflect.getMetadata('path', handler);

    expect(path).toBe('for-contact');
  });
});
